// app/kementerian/monitoring/page.tsx
'use client';
...
const MonitoringDistribusiPage = () => {
  ...
  const deliveryTrips = useMemo(() => [
    {
      ...
      // Posisi di tengah route (65% progress)
    ...
    },
    {
      ...
      // Sudah sampai sekolah (100% progress)
    ...
    },
    {
      ...
      // Sudah selesai di sekolah (100% progress)
  ...
  // Fetch real routes using OSRM (Open Source Routing Machine)
  useEffect(() => {
    if (!mapReady || Object.keys(routes).length > 0) return;

    const fetchRealRoutes = async () => {
      const newRoutes: any = {};
      const newRouteInfo: any = {};
      
      for (const trip of deliveryTrips) {
        try {
          // OSRM Public API - More stable, no auth needed
          const url = `https://router.project-osrm.org/route/v1/driving/${trip.kitchenLng},${trip.kitchenLat};${trip.schoolLng},${trip.schoolLat}?overview=full&geometries=geojson`;
          ...
          if (data.code === 'Ok' && data.routes && data.routes[0]) {
            ...
            newRoutes[trip.id] = leafletCoords;
            
            // Extract route info
            newRouteInfo[trip.id] = {
              distance: (route.distance / 1000).toFixed(1), // to km
              duration: Math.round(route.duration / 60), // to minutes
              hasTraffic: false
            };
            
            console.log(`✅ Route fetched for ${trip.id}:`, {
              distance: newRouteInfo[trip.id].distance,
              points: leafletCoords.length
            });
          } else {
            console.warn(`⚠️ Failed to get route for ${trip.id}, using fallback`);
            // Fallback to straight line
            newRoutes[trip.id] = [
              [trip.kitchenLat, trip.kitchenLng],
              [trip.schoolLat, trip.schoolLng]
            ];
            newRouteInfo[trip.id] = {
      ...
      console.log('🗺️ All routes fetched:', Object.keys(newRoutes).length);
      setRoutes(newRoutes);
      setRouteInfo(newRouteInfo);
    };

    fetchRealRoutes();
  }, [mapReady, deliveryTrips]);
  ...
  }, [deliveryTrips, selectedStatus, selectedProvince, searchQuery]);

  const provinces = useMemo(() => [...new Set(deliveryTrips.map(t => t.province))].sort(), [deliveryTrips]);
  ...
  return (
    <MinistryLayout currentPage="monitoring">
      <div className="p-4 md:p-6 lg:p-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4 mb-6">
          <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
            <div className="p-2.5 rounded-lg bg-blue-600 w-fit"><Truck className="w-5 h-5 text-white" /></div>
            <p className="text-xs font-medium text-gray-600 mt-3 mb-1">TOTAL PENGIRIMAN</p>
            <p className="text-2xl font-bold text-gray-900">{stats.total}</p>
          </div>
          <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
            <div className="p-2.5 rounded-lg bg-purple-600 w-fit"><PlayCircle className="w-5 h-5 text-white" /></div>
            <p className="text-xs font-medium text-gray-600 mt-3 mb-1">DALAM PERJALANAN</p>
            <p className="text-2xl font-bold text-gray-900">{stats.onTheWay}</p>
          </div>
          <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
            <div className="p-2.5 rounded-lg bg-green-600 w-fit"><CheckCircle className="w-5 h-5 text-white" /></div>
            <p className="text-xs font-medium text-gray-600 mt-3 mb-1">SUDAH TIBA</p>
            <p className="text-2xl font-bold text-gray-900">{stats.arrived}</p>
          </div>
          <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
            <div className="p-2.5 rounded-lg bg-emerald-600 w-fit"><Package className="w-5 h-5 text-white" /></div>
            <p className="text-xs font-medium text-gray-600 mt-3 mb-1">SELESAI</p>
            <p className="text-2xl font-bold text-gray-900">{stats.distributed}</p>
          </div>
          <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
            <div className="p-2.5 rounded-lg bg-gray-600 w-fit"><Clock className="w-5 h-5 text-white" /></div>
            <p className="text-xs font-medium text-gray-600 mt-3 mb-1">MENUNGGU</p>
            <p className="text-2xl font-bold text-gray-900">{stats.pending}</p>
          </div>
          <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
            <div className="p-2.5 rounded-lg bg-orange-600 w-fit"><TrendingUp className="w-5 h-5 text-white" /></div>
            <p className="text-xs font-medium text-gray-600 mt-3 mb-1">TRAYS TERKIRIM</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className={`lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-[500px] lg:h-[650px] ${selectedTrip ? 'pointer-events-none' : ''}`}>
            ...
            <div className="flex-1 min-h-0">
              {mapReady && (
                <MapContainer center={[-6.3, 107.0]} zoom={11} style={{ height: '100%', width: '100%' }}>
                  <TileLayer attribution='&copy; OpenStreetMap' url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
                  
                  {/* Route Lines - Real routing from Mapbox */}
                  {filteredTrips.map(trip => {
                    const routeCoords = routes[trip.id];
                    if (!routeCoords) return null;
                    
                    const info = routeInfo[trip.id];
                    ...
                    return (
                      <Polyline 
                        ...
                        pathOptions={{ 
                          ...
                          weight: 4, 
                          opacity: 0.7,
                          dashArray: trip.status === 'LOADED' || trip.status === 'PENDING' ? '10, 10' : undefined
                        }} 
                      >
                        {info && (
                          <Popup>
                            <div className="p-2">
                              <p className="font-bold text-xs">Route Info</p>
                              <p className="text-xs">Distance: {info.distance} km</p>
                              {info.duration && <p className="text-xs">Duration: ~{info.duration} min</p>}
                            </div>
                          </Popup>
                        )}
                      </Polyline>
                    ...
                    >
                      <Popup>
                        <div className="p-2">
                          ...
                          {routeInfo[trip.id] && (
                            <>
                              <p className="text-xs mt-1">📍 {routeInfo[trip.id].distance} km</p>
                              {routeInfo[trip.id].duration && (
                                <p className="text-xs">⏱️ ~{routeInfo[trip.id].duration} min</p>
                              )}
                            </>
                          )}
                        </div>
                    ...
                    >
                      ...
                      {trip.status === 'ON_THE_WAY' && (
                        <div className="mt-2">
                          ...
                          <p className="text-xs text-gray-500 mt-1">⏱️ {trip.timeRemaining}</p>
                        </div>
                      )}
                    </div>
                  ))}
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div>
                  <h4 className="font-bold text-gray-900 mb-3 flex items-center gap-2">
                    <Users className="w-5 h-5 text-[#D0B064]" />
                    Informasi Driver
                  </h4>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="font-semibold text-purple-600">⏱️ {selectedTrip.timeRemaining}</span>
                      </div>
                    </div>
                  </div>
                )}
...
export default MonitoringDistribusiPage;