// components/layout/DapurLayout.tsx
'use client';
...
const DapurLayout = ({ children, currentPage = 'dashboard' }: DapurLayoutProps) => {
  ...
  // Data dapur - bisa dari API/localStorage berdasarkan user yang login
  ...
  // Bisa fetch data dapur dari API saat component mount
  useEffect(() => {
    ...
    if (userData) {
      try {
        ...
        if (user.dapur) {
          setDapurInfo({
            nama: user.dapur.nama || 'Dapur Karawang',
            kota: user.dapur.kota || 'Karawang',
            pic: user.nama || 'Bu Siti Aminah',
            kode: user.dapur.kode || 'DPR-KRW-001'
          });
        }
      } catch (error) {
  ...
  // Navigation untuk Dapur/Produksi
  const navigation = [
    ...
    { id: 'scan', name: ' QR Checkpoint Scanner', icon: QrCodeIcon, path: '/dapur/scan' },
          ...
          <div className="mx-3 mt-4 mb-2 bg-white/5 rounded-lg p-3 border border-white/10">
            <div className="flex items-start gap-2 mb-2">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs text-gray-400">Lokasi Dapur</p>
          ...
          <div className="flex items-center justify-between w-full">
            ...
            <div className="flex items-center gap-3 bg-gradient-to-r from-[#D0B064] to-[#C9A355] px-6 py-2.5 rounded-full shadow-lg">
              ...
              <div className="text-left">
                <p className="text-xs text-white/80 font-medium">{dapurInfo.kode}</p>
                <p className="font-bold text-white tracking-wide leading-tight">{dapurInfo.nama}</p>
              </div>
            </div>
          </div>
        </header>
